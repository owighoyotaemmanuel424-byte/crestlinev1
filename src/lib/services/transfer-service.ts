...
import { ForbiddenError, NotFoundError, ValidationError, InsufficientBalanceError, TransferError, DailyLimitExceededError, MonthlyLimitExceededError } from '../utils/errors';
import { LedgerService } from './ledger-service';
import { Decimal } from '@prisma/client/runtime/library';
import type { User, Account, Transfer, TransferStatus, TransferType, Role, Journal } from '@prisma/client';
...
const TRANSFER_CONFIG = {
  MIN_TRANSFER_AMOUNT: new Decimal(1), MAX_TRANSFER_AMOUNT: new Decimal(1000000),
  DAILY_LIMIT: new Decimal(50000), MONTHLY_LIMIT: new Decimal(500000),
  MAX_DESCRIPTION_LENGTH: 500, DEFAULT_CURRENCY: 'USD', ALLOW_INTERNAL_TRANSFERS: true,
  ALLOW_EXTERNAL_TRANSFERS: true, REQUIRE_APPROVAL_AMOUNT: new Decimal(50000),
  FEE_PERCENTAGE: new Decimal(0.01), FEE_MINIMUM: new Decimal(1), FEE_MAXIMUM: new Decimal(100),
} as const;

export interface CreateTransferData {
  fromUserId: string; fromAccountId: string; toUserId: string; toAccountId: string;
  amount: number | string | Decimal; currency?: string; description?: string;
  reference?: string; idempotencyKey?: string; metadata?: Record<string, unknown>;
}

export interface TransferResult {
  transfer: Transfer & { fromUser: Pick<User, 'id' | 'email' | 'firstName' | 'lastName'>;
    toUser: Pick<User, 'id' | 'email' | 'firstName' | 'lastName'>;
    fromAccount: Pick<Account, 'id' | 'accountNumber' | 'balance' | 'availableBalance'>;
    toAccount: Pick<Account, 'id' | 'accountNumber' | 'balance' | 'availableBalance'>; journal?: Journal | null; };
}

export interface TransferListResult { transfers: Transfer[]; total: number; page: number; limit: number; totalPages: number; }

export interface TransferStats {
  totalTransfers: number; totalAmount: number; byStatus: Record<TransferStatus, number>;
  byType: Record<TransferType, number>; averageAmount: number; pendingApproval: number;
}
...
export class TransferService {
  static async createTransfer(data: CreateTransferData, actingUserId?: string): Promise<TransferResult> {
    ...
    if (actingUserId && actingUserId !== data.fromUserId) {
      ...
      if (!actingUser || !['ADMIN', 'SUPER_ADMIN'].includes(actingUser.role)) throw new ForbiddenError('You can only transfer from your own account');
    }
    ...
    if (availableBalance.lessThan(amount)) throw new InsufficientBalanceError(fromAccount.id, amount.toNumber(), availableBalance.toNumber());
    ...
    const dailyTotal = toDecimal(dailyTransfers._sum.amount || 0); const newDailyTotal = dailyTotal.plus(amount);
    if (newDailyTotal.greaterThan(TRANSFER_CONFIG.DAILY_LIMIT)) throw new DailyLimitExceededError(data.fromUserId, TRANSFER_CONFIG.DAILY_LIMIT.toNumber(), newDailyTotal.toNumber());
    ...
    const monthlyTotal = toDecimal(monthlyTransfers._sum.amount || 0); const newMonthlyTotal = monthlyTotal.plus(amount);
    if (newMonthlyTotal.greaterThan(TRANSFER_CONFIG.MONTHLY_LIMIT)) throw new MonthlyLimitExceededError(data.fromUserId, TRANSFER_CONFIG.MONTHLY_LIMIT.toNumber(), newMonthlyTotal.toNumber());
    ...
    if (availableBalance.lessThan(totalDeduction)) throw new InsufficientBalanceError(fromAccount.id, totalDeduction.toNumber(), availableBalance.toNumber());
    return await prisma.$transaction(async (tx) => {
      const transfer = await tx.transfer.create({
        data: { reference, fromUserId: data.fromUserId, toUserId: data.toUserId, fromAccountId: data.fromAccountId, toAccountId: data.toAccountId,
          amount: amount, fee: fee, totalAmount: totalDeduction, currency: data.currency || TRANSFER_CONFIG.DEFAULT_CURRENCY,
          description: data.description || null, status: requiresApproval ? 'PENDING' : 'COMPLETED' as TransferStatus,
          type: data.fromUserId === data.toUserId ? 'INTERNAL' : 'EXTERNAL' as TransferType, idempotencyKey, metadata: data.metadata || null },
        include: { fromUser: { select: { id: true, email: true, firstName: true, lastName: true } },
          ...
          toAccount: { select: { id: true, accountNumber: true, balance: true, availableBalance: true } }, journal: true },
      });
      await LedgerService.createJournal({ reference: generateReference('TRF-JNL'),
        description: 'Transfer ' + reference + ' - ' + amount.toString() + ' ' + (data.currency || 'USD'),
        entries: [{ accountId: data.fromAccountId, entryType: 'DEBIT', amount: totalDeduction,
          description: 'Transfer to ' + toAccount.accountNumber + ' - Amount: ' + amount.toString() + ', Fee: ' + fee.toString(), transactionId: transfer.id },
        ...
        transferId: transfer.id, metadata: { transferId: transfer.id, fromAccountId: data.fromAccountId, toAccountId: data.toAccountId, amount: amount.toString(), fee: fee.toString(), currency: data.currency || 'USD' },
      }, actingUserId);
      if (!requiresApproval) {
      ...
      await tx.auditLog.create({ data: { actorId: actingUserId || data.fromUserId, action: 'CREATE', resourceType: 'TRANSFER', resourceId: transfer.id,
      ...
      if (requiresApproval) {
        ...
        for (const admin of admins) await tx.notification.create({ data: { userId: admin.id, title: 'Transfer Approval Required',
          message: 'Transfer of ' + amount.toString() + ' ' + (data.currency || 'USD') + ' from ' + fromUser.email + ' to ' + toUser.email + ' requires approval',
          type: 'WARNING', category: 'TRANSFER', isRead: false,
          metadata: { transferId: transfer.id, fromUserId: data.fromUserId, toUserId: data.toUserId, amount: amount.toString() } } });
        await tx.notification.create({ data: { userId: data.fromUserId, title: 'Transfer Pending Approval',
          message: 'Your transfer of ' + amount.toString() + ' ' + (data.currency || 'USD') + ' to ' + toUser.email + ' is pending approval',
          type: 'INFO', category: 'TRANSFER', isRead: false, metadata: { transferId: transfer.id } } });
      } else {
        await tx.notification.create({ data: { userId: data.fromUserId, title: 'Transfer Completed',
          message: 'Your transfer of ' + amount.toString() + ' ' + (data.currency || 'USD') + ' to ' + toUser.email + ' has been completed',
          type: 'SUCCESS', category: 'TRANSFER', isRead: false, metadata: { transferId: transfer.id } } });
        await tx.notification.create({ data: { userId: data.toUserId, title: 'Transfer Received',
  ...
  static async listAll(actingUserId: string, page: number = 1, limit: number = 20, type?: TransferType, status?: TransferStatus, startDate?: Date, endDate?: Date, userId?: string): Promise<TransferListResult> {
    ...
    const where: Record<string, unknown> = {}; if (type) where.type = type; if (status) where.status = status;
  ...
  static async approve(id: string, actingUserId: string, notes?: string): Promise<TransferResult> {
    ...
    const totalAmount = toDecimal(transfer.totalAmount); const availableBalance = toDecimal(transfer.fromAccount.availableBalance);
    ...
    return await prisma.$transaction(async (tx) => {
      const approvedTransfer = await tx.transfer.update({ where: { id }, data: { status: 'APPROVED' as TransferStatus, approvedById: actingUserId, approvedAt: new Date(), approvalNotes: notes || null },
        include: { fromUser: { select: { id: true, email: true, firstName: true, lastName: true } },
          toUser: { select: { id: true, email: true, firstName: true, lastName: true } },
          fromAccount: { select: { id: true, accountNumber: true, balance: true, availableBalance: true } },
          toAccount: { select: { id: true, accountNumber: true, balance: true, availableBalance: true } }, journal: true } });
  ...
  static async reject(id: string, actingUserId: string, reason: string): Promise<TransferResult> {
    ...
    const rejectedTransfer = await prisma.transfer.update({ where: { id }, data: { status: 'REJECTED' as TransferStatus, rejectedById: actingUserId, rejectedAt: new Date(), rejectionReason: reason },
      include: { fromUser: { select: { id: true, email: true, firstName: true, lastName: true } },
        toUser: { select: { id: true, email: true, firstName: true, lastName: true } },
        fromAccount: { select: { id: true, accountNumber: true, balance: true, availableBalance: true } },
        toAccount: { select: { id: true, accountNumber: true, balance: true, availableBalance: true } }, journal: true } });
