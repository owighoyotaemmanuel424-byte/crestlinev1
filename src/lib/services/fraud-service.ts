...
export class FraudService {
  static async check(data: FraudCheckData, actingUserId?: string): Promise<FraudCheckResult> {
    ...
    } else if (amount.greaterThanOrEqual(FRAUD_CONFIG.SUSPICIOUS_THRESHOLD)) {
      riskScore += 20;
    }
    ...
    const newTotal = total24h.plus(amount);
    if (newTotal.greaterThan(FRAUD_CONFIG.VELOCITY_THRESHOLD)) {
      riskScore += 25;
    ...
    if (isBlocked) {
      ...
      for (const admin of admins) {
        await prisma.notification.create({
          data: {
            userId: admin.id,
            title: 'Fraud Alert - Suspicious Activity',
  ...
  static async resolve(id: string, actingUserId: string, resolution: string, isFalsePositive: boolean): Promise<FraudCheckResult> {
    ...
    const updatedAlert = await prisma.fraudAlert.update({
      where: { id },
      data: { status: 'RESOLVED' as FraudStatus, resolvedById: actingUserId, resolvedAt: new Date(), resolution, isFalsePositive },
      include: { user: { select: { id: true, email: true, firstName: true, lastName: true } }, account: { select: { id: true, accountNumber: true } }, transaction: { select: { id: true, reference: true } } },
    });
    await prisma.auditLog.create({ data: { actorId: actingUserId, action: 'RESOLVE', resourceType: 'FRAUD_ALERT', resourceId: alert.id, oldValues: { status: alert.status }, newValues: { status: 'RESOLVED', resolution, isFalsePositive }, status: 'SUCCESS' } });
    if (isFalsePositive) {
  ...
  static async getStats(actingUserId: string): Promise<FraudStats> {
    ...
    const typeCounts = await prisma.fraudAlert.groupBy({ by: ['riskLevel'], _count: { _all: true } });
    ...
    const averageResponseTime = resolvedAlerts.length > 0 ? resolvedAlerts.reduce((sum, a) => a.resolvedAt ? sum + (a.resolvedAt.getTime() - a.createdAt.getTime()) / 1000 : sum, 0) / resolvedAlerts.length : 0;
