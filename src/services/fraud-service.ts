...
export class FraudService {
  ...
  static async getById(id: string, actingUserId: string): Promise<FraudCheckResult> {
    ...
    return {
      alert,
      riskLevel: alert.riskLevel as RiskLevel,
  ...
  static async listByUser(
    ...
    startDate?: Date,
    endDate?: Date
  ): Promise<{ alerts: FraudAlert[]; total: number; page: number; limit: number; totalPages: number }> {
  ...
  static async resolve(
    ...
    const updatedAlert = await prisma.fraudAlert.update({
      where: { id },
      data: {
        status: 'RESOLVED' as FraudStatus,
    ...
    await prisma.auditLog.create({
      data: {
        ...
        newValues: { status: 'RESOLVED', resolution, isFalsePositive },
        status: 'SUCCESS',
      },
    });
    ...
    return {
      alert: updatedAlert,
      riskLevel: updatedAlert.riskLevel as RiskLevel,
      riskScore: updatedAlert.riskScore,
      isSuspicious: updatedAlert.isSuspicious,
      isBlocked: updatedAlert.isBlocked,
      recommendations: updatedAlert.recommendations as string[],
    };
  }
  ...
  static async getStats(actingUserId: string): Promise<FraudStats> {
    ...
    const riskLevelCounts = await prisma.fraudAlert.groupBy({
      by: ['riskLevel'],
      _count: { _all: true },
    });
    ...
    const totalFlaggedAmount = flaggedAlerts
      .reduce((sum, a) => sum.plus(toDecimal(a.amount)), new Decimal(0))
      .toNumber();

    const blockedAlerts = await prisma.fraudAlert.findMany({
      where: { isBlocked: true },
      select: { amount: true },
    });

    const totalBlockedAmount = blockedAlerts
      .reduce((sum, a) => sum.plus(toDecimal(a.amount)), new Decimal(0))
      .toNumber();
