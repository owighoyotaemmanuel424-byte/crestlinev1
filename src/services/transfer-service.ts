...
const TRANSFER_CONFIG = {
  ...
  FEE_PERCENTAGE: new Decimal(0.01), // 1%
...
export class TransferService {
  ...
  static async createTransfer(
    ...
    // Authorization check - only user can transfer from their own account
    ...
    if (fromAccount.userId !== data.fromUserId) {
      throw new ForbiddenError('From account does not belong to from user');
    }
    ...
    if (toAccount.userId !== data.toUserId) {
      throw new ForbiddenError('To account does not belong to to user');
    }

    // Check account status
    if (fromAccount.status !== 'ACTIVE') {
      throw new ValidationError(`From account is ${fromAccount.status.toLowerCase()}`);
    }
    if (toAccount.status !== 'ACTIVE') {
      throw new ValidationError(`To account is ${toAccount.status.toLowerCase()}`);
    }
    ...
    if (amount.greaterThan(TRANSFER_CONFIG.MAX_TRANSFER_AMOUNT)) {
      throw new ValidationError(
        `Amount exceeds maximum transfer limit of ${TRANSFER_CONFIG.MAX_TRANSFER_AMOUNT.toString()}`
      );
    }
    ...
    if (data.idempotencyKey) {
      ...
      if (existingTransfer) {
        return {
          transfer: existingTransfer as Transfer & {
            fromUser: any;
            toUser: any;
            fromAccount: any;
            toAccount: any;
    ...
    const dailyTransfers = await prisma.transfer.aggregate({
      where: {
        fromUserId: data.fromUserId,
        createdAt: { gte: today },
        status: { in: ['PENDING', 'COMPLETED', 'APPROVED'] as TransferStatus[] },
      },
      _sum: { amount: true },
    });
    ...
    if (newDailyTotal.greaterThan(TRANSFER_CONFIG.DAILY_LIMIT)) {
      throw new DailyLimitExceededError(
        data.fromUserId,
        TRANSFER_CONFIG.DAILY_LIMIT.toNumber(),
    ...
    const monthlyTransfers = await prisma.transfer.aggregate({
      where: {
        fromUserId: data.fromUserId,
    ...
    if (newMonthlyTotal.greaterThan(TRANSFER_CONFIG.MONTHLY_LIMIT)) {
      throw new MonthlyLimitExceededError(
        data.fromUserId,
        TRANSFER_CONFIG.MONTHLY_LIMIT.toNumber(),
        newMonthlyTotal.toNumber()
      );
    }

    // Check if approval is required using Decimal comparison
    ...
    const fee = feeAmount.lessThan(TRANSFER_CONFIG.FEE_MINIMUM)
      ? TRANSFER_CONFIG.FEE_MINIMUM
      : feeAmount.greaterThan(TRANSFER_CONFIG.FEE_MAXIMUM)
        ? TRANSFER_CONFIG.FEE_MAXIMUM
        : feeAmount;
    ...
    return await prisma.$transaction(async (tx) => {
        ...
        entries: [
          {
            ...
            description: `Transfer to ${toAccount.accountNumber} - Amount: ${amount.toString()}, Fee: ${fee.toString()}`,
      ...
      // Update account balances if not pending approval
      if (!requiresApproval) {
      ...
      if (requiresApproval) {
        const admins = await tx.user.findMany({
          where: { role: { in: ['ADMIN', 'SUPER_ADMIN'] as Role[] } },
        });
        ...
        // Send notification to from user
        await tx.notification.create({
          data: {
  ...
  static async approve(id: string, actingUserId: string, notes?: string): Promise<TransferResult> {
    ...
    const actingUser = await prisma.user.findUnique({ where: { id: actingUserId } });
    if (!actingUser || !['ADMIN', 'SUPER_ADMIN'].includes(actingUser.role)) {
      throw new ForbiddenError('Only administrators can approve transfers');
    }
    ...
    // Check that accounts are still valid
    if (transfer.fromAccount.status !== 'ACTIVE') {
      throw new ValidationError('From account is no longer active');
    }
    if (transfer.toAccount.status !== 'ACTIVE') {
      throw new ValidationError('To account is no longer active');
    }

    // Check that from account still has sufficient balance using Decimal comparison
    ...
    return await prisma.$transaction(async (tx) => {
      ...
      await tx.account.update({
        ...
        data: {
          balance: {
            increment: toDecimal(transfer.amount),
          },
          availableBalance: {
            increment: toDecimal(transfer.amount),
          },
      ...
      await tx.notification.create({
        data: {
          ...
          message: `Your transfer of ${toDecimal(transfer.amount).toString()} ${transfer.currency} to ${transfer.toUser.email} has been approved`,
          type: 'SUCCESS',
          category: 'TRANSFER',
          isRead: false,
          metadata: { transferId: transfer.id },
      ...
      await tx.notification.create({
        data: {
          ...
          title: 'Transfer Received',
          message: `You have received ${toDecimal(transfer.amount).toString()} ${transfer.currency} from ${transfer.fromUser.email}`,
          type: 'SUCCESS',
          category: 'TRANSFER',
          isRead: false,
          metadata: { transferId: transfer.id },
  ...
  static async reject(id: string, actingUserId: string, reason: string): Promise<TransferResult> {
    ...
    const actingUser = await prisma.user.findUnique({ where: { id: actingUserId } });
    if (!actingUser || !['ADMIN', 'SUPER_ADMIN'].includes(actingUser.role)) {
      throw new ForbiddenError('Only administrators can reject transfers');
    }
    ...
    await prisma.notification.create({
      data: {
        ...
        message: `Your transfer of ${toDecimal(transfer.amount).toString()} ${transfer.currency} to ${transfer.toUser.email} has been rejected. Reason: ${reason}`,
        type: 'ERROR',
        category: 'TRANSFER',
        isRead: false,
        metadata: { transferId: transfer.id, reason },
      },
    });
  ...
  static async getStats(actingUserId: string): Promise<TransferStats> {
    ...
    for (const group of statusCounts) {
      byStatus[group.status as TransferStatus] = group._count._all;
    }
    ...
    const typeCounts = await prisma.transfer.groupBy({
      by: ['type'],
      _count: { _all: true },
    });

    for (const group of typeCounts) {
      byType[group.type as TransferType] = group._count._all;
    }

    // Calculate total amount using Decimal arithmetic
    const allTransfers = await prisma.transfer.findMany({
      where: { status: { in: ['COMPLETED', 'APPROVED'] as TransferStatus[] } },
      select: { amount: true },
    });

    const totalAmount = allTransfers
      .reduce((sum, t) => sum.plus(toDecimal(t.amount)), new Decimal(0))
      .toNumber();

    // Calculate average amount using Decimal arithmetic
    const averageAmount = allTransfers.length > 0
      ? allTransfers.reduce((sum, t) => sum.plus(toDecimal(t.amount)), new Decimal(0)).div(allTransfers.length).toNumber()
      : 0;

    // Count pending approval
    const pendingApproval = await prisma.transfer.count({
