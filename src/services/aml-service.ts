...
export class AMLService {
  ...
  static async approve(checkId: string, actingUserId: string, notes?: string): Promise<AMLCheckResult> {
    ...
    await prisma.auditLog.create({
      data: { actorId: actingUserId, action: 'APPROVE', resourceType: 'AML_CHECK', resourceId: check.id, oldValues: { status: check.status }, newValues: { status: 'APPROVED', notes }, status: 'SUCCESS' },
    });
    await prisma.notification.create({
      data: { userId: check.userId, title: 'AML Check Approved', message: 'Your AML check has been approved. Reference: ' + check.id, type: 'SUCCESS', category: 'COMPLIANCE', isRead: false, metadata: { checkId: check.id } },
    });
  ...
  static async reject(checkId: string, actingUserId: string, reason: string): Promise<AMLCheckResult> {
    ...
    await prisma.auditLog.create({
      data: { actorId: actingUserId, action: 'REJECT', resourceType: 'AML_CHECK', resourceId: check.id, oldValues: { status: check.status }, newValues: { status: 'REJECTED', reason }, status: 'SUCCESS' },
    });
    await prisma.notification.create({
      data: { userId: check.userId, title: 'AML Check Rejected', message: 'Your AML check has been rejected. Reason: ' + reason, type: 'ERROR', category: 'COMPLIANCE', isRead: false, metadata: { checkId: check.id, reason } },
    });
    await prisma.user.update({
      where: { id: check.userId },
      data: { amlFlagged: true, amlFlaggedAt: new Date(), amlFlaggedReason: reason },
    });
    return { check: updatedCheck, riskLevel: updatedCheck.riskLevel as RiskLevel, riskScore: updatedCheck.riskScore, requiresReview: updatedCheck.requiresReview, requiresApproval: updatedCheck.requiresApproval };
  }
